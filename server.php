...
//------------------------------------------------------------------------------------------
	...
	if($room!="" && $user!=''){
		...
		for($i=0;$i<count($lu);$i++){
			if( strpos($lu[$i],($user))!=false ||(time()- data2ts(substr($lu[$i],0,19))) >30000){
				unset($lu[$i]);
		...
		if(strpos($listuser,substr($user,0,-34))==false){
	...
	case "html":
		...
		break;	
	case "last":
		echo @array_pop(file("rooms/".get_sb()."/".$room));
		break;	
